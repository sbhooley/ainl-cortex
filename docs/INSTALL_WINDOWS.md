...
| `/reload-plugins` still shows an **old version** (e.g. no `v0.4.x` in banner) | Claude often loads **`plugins\cache\ainl-local\ainl-cortex\<version>`** — a frozen marketplace copy. **macOS** git clones update in place; **Windows** reload alone may not fetch GitHub. **Fix (one-time):** clone live: `git clone https://github.com/sbhooley/ainl-cortex.git %USERPROFILE%\.claude\plugins\ainl-cortex` then `setup.cmd -PythonOnly`, `/reload-plugins`. **v0.4.5+** auto-clones/pulls the live tree on SessionStart/MCP and repoints `installed_plugins.json`. Confirm banner: `[AINL Cortex vX.Y.Z]`. |
